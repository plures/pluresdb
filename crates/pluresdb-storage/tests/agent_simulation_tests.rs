...
async fn test_long_running_agent_simulation() {
    ...
    println!(
        "Starting long-running agent simulation for {} minutes",
        runtime_minutes
    );
    ...
    while start_time.elapsed() < Duration::from_secs(runtime_minutes * 60) {
        ...
        println!(
            "Recovered {} operations from previous sessions",
            entries_before.len()
        );
        ...
        println!(
            "Session {} completed: {} operations written",
            crash_count + 1,
            session_ops
        );
    ...
    println!(
        ...
        total_operations / (crash_count + 1)
    );
...
async fn test_memory_bounded_growth() {
    ...
    println!(
        "Testing bounded memory growth with {} operations",
        operations_count
    );
    ...
    for i in 0u64..operations_count {
        ...
        if i > 0 && i % compaction_threshold == 0 {
            ...
            println!(
                "After compaction at i={}: {} entries remain",
                i,
                entries.len()
            );
    ...
    println!(
        "Growth ratio: {:.2}x",
        final_entries.len() as f64 / operations_count as f64
    );
...
async fn test_concurrent_agent_workers() {
    ...
    println!(
        "Testing {} concurrent agent workers, {} ops each",
