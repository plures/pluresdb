...
## ▶️ NEXT OPEN STAGE: S4e gateway-restart LIVE-VERIFY + epic close. Native integration is DONE (see below): canonical main fast-forwarded to reactive, canonical .node rebuilt fresh (reactive), and all three S4 gates PASS through the plugin's real loader/dist. Remaining = restart gateway so the live plugin loads the fresh reactive .node, confirm memory capability loads with no native error, then close epic #1/#6 and disable this driver.

## ✅ S4 NATIVE INTEGRATION DONE (2026-07-02 ~13:15) — only live restart-verify + close remain
- Canonical repo `C:\Projects\pluresdb` and the worktree SHARE ONE .git; `main` was the exact merge-base of `feat/native-memory-reactive` => clean **fast-forward** `0ec9523 -> 83c8469` put S2(subscribe/unsubscribe)+S3(subscribePx)+PxEventJs/SyncEventJs onto the crate the plugin links via `file:../pluresdb/crates/pluresdb-node`.
- Rebuilt canonical `.node`: `npm run build` (napi build --platform --release) **EXIT=0** (1m37s). Fresh artifact 36.5MB @13:14 (was stale 57.6MB @7/1). Generated index.d.ts REACTIVE_DTS_HITS=11 (subscribe/unsubscribe/subscribePx/PxEventJs/SyncEventJs).
- Verified against a NON-DESTRUCTIVE temp copy of the real migrated-store:
  - **S4_NATIVE_GATE PASS** — fresh .node: PluresDatabase+compressText+countTokens + proto.subscribe/subscribePx/unsubscribe all `function`.
  - **S4_PLUGIN_RESOLVE_GATE PASS** — plugin `require.resolve('@plures/pluresdb-native')` OK via pnpm virtual store `.pnpm/@plures+pluresdb-native@fil_34809d5030a4b71bc4393067b4a9d311/...`. That copy has NO `.node` sibling (pnpm copied only index.js/d.ts), so the loader's `ensureNativeLibraryPath` fallback (`nativeCrateRoots()` walk / PLURESDB_NODE_DIR) correctly sets `NAPI_RS_NATIVE_LIBRARY_PATH` -> canonical fresh .node. **No symlink repair needed** — pnpm-no-.node quirk is handled by design.
  - **S4_DIST_E2E_GATE PASS** — importing the BUILT dist `C:\Projects\plureslm-openclaw\dist\pluresdb.js`, constructing `PluresLmStore` on the temp copy, `store.recall` runs (dist's own loader loads native; env auto-set to canonical .node), reactive subscribe/subscribePx/unsubscribe present, `subscribePx` smoke returns numeric id + clean unsubscribe. `headroomAvailable()`=true.
- Production wiring confirmed: openclaw.json `plugins.load.paths=[C:\Projects\plureslm-openclaw]`, `plugins.entries.plureslm` enabled (dbPath C:\Users\kbristol\.pluresLM\migrated-store), `plugins.slots.memory=plureslm`. Gateway loads the plugin from source; the fresh reactive .node is what it will pick up on restart.
- HONEST SCOPE NOTE: the plugin STORE WRAPPER does not yet CALL subscribePx (consolidation stays pull/tick; S3 subscribePx is honest POST-write observe/report). Native reactive is available+reachable end-to-end; ACTIVE push-wiring inside the plugin is a separate optional follow-on, NOT part of E1-E3 delivery. Do NOT fake a push path.
...
- [~] S4 integrate: **NATIVE INTEGRATION DONE 2026-07-02 ~13:15** (canonical FF to reactive + fresh reactive .node built EXIT=0 + S4_NATIVE_GATE/S4_PLUGIN_RESOLVE_GATE/S4_DIST_E2E_GATE all PASS on real-store temp copy). Remaining sub-step **S4e = restart gateway -> live-verify plugin memory capability loads with no native error -> close epic #1/#6 -> disable native-epic-driver.**
