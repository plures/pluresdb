...
- ✅ **JSR Package**: `deno.json` now advertises `@plures/pluresdb` so Deno users can `deno add @plures/pluresdb`
- ✅ **Pure Deno Entry**: `mod.ts` re-exports only browser/Deno-safe modules without the Node wrapper
- ✅ **CLI Export**: `jsr:@plures/pluresdb/cli` maps to the native Deno CLI (`src/main.ts`)
- ✅ **Doc Updates**: README and installation guide document the Deno workflow alongside npm
