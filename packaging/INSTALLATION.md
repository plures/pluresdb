...
Install via the JSR registry to get the pure Deno build:

```bash
deno add @plures/pluresdb
```

Run the CLI directly from the package without installing a Node wrapper:

```bash
deno run -A --unstable-kv jsr:@plures/pluresdb/cli serve
```

Or execute the library inline for scripting:

```bash
deno run -A --unstable-kv jsr:@plures/pluresdb/mod.ts
```

**Requirements:**

- Deno 2.5 or later (unstable KV enabled)
