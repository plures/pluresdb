...
const DEMO_DATA = {
    users: [
        ...
        { id: 4, name: 'Alice Brown', email: 'alice@example.com', age: 28, created_at: '2024-01-04 13:00:00' },
...
class SQLiteCompatibilityDemo {
    ...
    async testSQLQueries() {
        ...
        const queries = [
            ...
            { name: 'JOIN Query', sql: 'SELECT u.name, p.title FROM users u JOIN posts p ON u.id = p.user_id' },
            ...
            { name: 'CASE Statement', sql: 'SELECT name, CASE WHEN age < 30 THEN "Young" ELSE "Adult" END as category FROM users' }
        ];
