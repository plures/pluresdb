...
async function runMemoryBenchmarks() {
  ...
  try {
    ...
    if ((globalThis as any).subscriptions) {
      ((globalThis as any).subscriptions as Array<() => void>).forEach(unsubscribe => unsubscribe());
...
async function runMemoryLeakTests() {
  ...
  try {
    ...
    console.log(`Memory increase after subscription cycles: ${(memoryIncrease / 1024).toFixed(2)}KB`);
    
    if (memoryIncrease > 1024 * 1024) { // More than 1MB
    ...
    if (crudMemoryIncrease > 1024 * 1024) { // More than 1MB
