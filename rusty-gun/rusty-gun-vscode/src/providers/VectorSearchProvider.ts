...
export class VectorSearchProvider
  implements vscode.TreeDataProvider<VectorSearchItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<
    VectorSearchItem | undefined | null | void
  > = new vscode.EventEmitter<VectorSearchItem | undefined | null | void>();
  ...
  private async performSearch(query: string): Promise<void> {
    ...
    try {
      const threshold = vscode.workspace.getConfiguration("rusty-gun").get(
        "vectorSearchThreshold",
        0.3,
      );
      const limit = vscode.workspace.getConfiguration("rusty-gun").get(
        "maxResults",
        100,
      );

      this.searchResults = await this.client.searchVectors(
        query,
        limit,
        threshold,
      );
