...
export class RustyGunProvider implements vscode.TreeDataProvider<RustyGunItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<RustyGunItem | undefined | null | void> = new vscode.EventEmitter<RustyGunItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<RustyGunItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    getChildren(element?: RustyGunItem): Thenable<RustyGunItem[]> {
        ...
        if (element.type === 'nodes') {
            // Node items
            return Promise.resolve(
                this.nodes.map(node => new RustyGunItem(
                    node.data?.name || node.data?.title || node.id,
                    `${node.data?.type || 'Unknown'} • ${new Date(node.created_at).toLocaleDateString()}`,
        ...
        if (element.type === 'relationships') {
            // Relationship items
            return Promise.resolve(
                this.relationships.map(rel => new RustyGunItem(
                    `${rel.from} → ${rel.to}`,
