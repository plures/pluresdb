...
export class RustyGunClient {
    ...
    async createRelationship(relationshipData: Partial<Relationship>): Promise<Relationship> {
        const response = await this.httpClient.post('/relationships', relationshipData);
        return response.data.data;
    }

    async updateRelationship(id: string, relationshipData: Partial<Relationship>): Promise<Relationship> {
    ...
    async searchVectors(query: string, limit = 5, threshold = 0.3): Promise<VectorSearchResult[]> {
    ...
    async addVectorText(id: string, text: string, metadata: any = {}): Promise<void> {
