<script lang="ts">
  ...
  let dark = false
  let activeTab: 'subscriptions' | 'payments' | 'usage' | 'analytics' | 'invoices' = 'subscriptions'
  ...
  let availablePlans = [
    ...
    },
    {
      ...
      currency: 'USD',
      interval: 'monthly' as const,
      features: ['Up to 100,000 nodes', '100GB storage', '10 users', 'Priority support', 'Advanced analytics'],
    ...
    },
    {
      ...
      currency: 'USD',
      interval: 'monthly' as const,
      features: ['Unlimited nodes', '1TB storage', 'Unlimited users', '24/7 support', 'Custom integrations'],
  ...
  function getStatusColor(status: string): string {
    switch (status) {
      ...
      case 'past_due': return 'var(--warning-color)'
      case 'trialing': return 'var(--primary-color)'
      case 'paid': return 'var(--success-color)'
      case 'pending': return 'var(--warning-color)'
      case 'failed': return 'var(--error-color)'
      case 'draft': return 'var(--muted-color)'
      ...
      {#if activeTab === 'subscriptions'}
        <div class="subscriptions-section">
          <div class="section-header">
            ...
            <button on:click={() => showPlanDialog = true} class="primary">
              Change Plan
            </button>
          </div>
            ...
            {#each subscriptions as subscription}
              ...
              >
                <div class="subscription-header">
                  ...
                  <div class="subscription-status" style="color: {getStatusColor(subscription.status)}">
                    {getStatusIcon(subscription.status)} {subscription.status}
                  </div>
                </div>
                <div class="subscription-details">
                  <div class="subscription-period">
                    <span>Current period: {formatTimestamp(subscription.currentPeriodStart)} - {formatTimestamp(subscription.currentPeriodEnd)}</span>
                ...
                <div class="subscription-actions">
                  {#if subscription.cancelAtPeriodEnd}
                    <button on:click|stopPropagation={() => reactivateSubscription(subscription.id)} class="small">
                      Reactivate
                    </button>
                  {:else}
                    <button on:click|stopPropagation={() => cancelSubscription(subscription.id)} class="small">
                      Cancel
                    </button>
                  {/if}
                </div>
        ...
        <div class="usage-section">
          ...
          <div class="usage-grid">
            <div class="usage-card">
              ...
              <div class="usage-bar">
                <div class="usage-fill" style="width: {getUsagePercentage(usage.nodes, selectedSubscription?.limits?.nodes || 1000)}%"></div>
              </div>
              <div class="usage-limit">
                Limit: {selectedSubscription?.limits?.nodes === -1 ? 'Unlimited' : selectedSubscription?.limits?.nodes?.toLocaleString() || '1,000'}
              </div>
            </div>
            ...
            <div class="usage-card">
              ...
              <div class="usage-bar">
                <div class="usage-fill" style="width: {getUsagePercentage(parseInt(usage.storage), parseInt(selectedSubscription?.limits?.storage || '1GB'))}%"></div>
              </div>
              <div class="usage-limit">
            ...
            <div class="usage-card">
              ...
              <div class="usage-bar">
                <div class="usage-fill" style="width: {getUsagePercentage(usage.users, selectedSubscription?.limits?.users || 1)}%"></div>
              </div>
              <div class="usage-limit">
                Limit: {selectedSubscription?.limits?.users === -1 ? 'Unlimited' : selectedSubscription?.limits?.users || 1}
              </div>
            </div>
            ...
            <div class="usage-card">
              ...
              <div class="usage-bar">
                <div class="usage-fill" style="width: {getUsagePercentage(usage.apiCalls, selectedSubscription?.limits?.apiCalls || 10000)}%"></div>
              </div>
              <div class="usage-limit">
                Limit: {selectedSubscription?.limits?.apiCalls === -1 ? 'Unlimited' : selectedSubscription?.limits?.apiCalls?.toLocaleString() || '10,000'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="analytics-section">
          ...
          <div class="analytics-grid">
            ...
            <div class="analytics-card">
              ...
              <div class="analytics-value">{formatCurrency(analytics.averageRevenuePerUser, 'USD')}</div>
            </div>
    ...
    <div class="dialog-overlay">
      <div class="dialog">
        ...
        <div class="dialog-actions">
          <button on:click={() => showPlanDialog = false} class="secondary">
            Cancel
          </button>
        </div>
      </div>
  ...
  {#if showPaymentDialog}
    <div class="dialog-overlay">
      <div class="dialog">
        ...
        <div class="dialog-actions">
          <button on:click={addPaymentMethod} disabled={!newPaymentMethod.cardNumber.trim() || !newPaymentMethod.name.trim()}>
            Add Payment Method
          </button>
          <button on:click={() => showPaymentDialog = false} class="secondary">
            Cancel
          </button>
        </div>
      </div>
...
<style>
  ...
  .subscriptions-list, .payment-methods-list, .invoices-list {
    display: flex;
    flex-direction: column;
    gap: 0.75rem;
  }
  
  .subscription-item, .payment-method-item, .invoice-item {
    padding: 1rem;
    border: 1px solid var(--pico-muted-border-color);
    border-radius: 8px;
  ...
  .subscription-item:hover, .payment-method-item:hover, .invoice-item:hover {
  ...
  .subscription-header, .payment-method-info, .invoice-header {
    display: flex;
    justify-content: space-between;
    align-items: center;
  ...
  .subscription-info, .payment-method-details, .invoice-info {
    display: flex;
    flex-direction: column;
    gap: 0.25rem;
  }
  
  .subscription-plan, .payment-method-type, .invoice-number {
    font-weight: 600;
    font-size: 1rem;
  }
  
  .subscription-price, .invoice-amount {
    font-size: 0.875rem;
    color: var(--pico-primary);
  }
  
  .subscription-details, .invoice-details {
    display: flex;
    flex-direction: column;
    gap: 0.25rem;
    font-size: 0.875rem;
  ...
  .subscription-actions, .payment-method-actions, .invoice-actions {
    display: flex;
    gap: 0.5rem;
  }
