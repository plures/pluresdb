  ...
  function getMarkdownBlocks(content: string): MarkdownBlock[] {
    ...
    const flushList = () => {
      if (currentList.length > 0) {
        blocks.push({ type: "list", items: currentList.map(formatInline) });
        currentList = [];
      }
    };
    ...
    for (const rawLine of lines) {
      ...
      if (line.startsWith("# ")) {
        flushList();
        blocks.push({ type: "heading", level: 1, html: formatInline(line.substring(2).trim()) });
        continue;
      }

      if (line.startsWith("## ")) {
        flushList();
        blocks.push({ type: "heading", level: 2, html: formatInline(line.substring(3).trim()) });
        continue;
      }

      if (line.startsWith("### ")) {
        flushList();
        blocks.push({ type: "heading", level: 3, html: formatInline(line.substring(4).trim()) });
        continue;
      }
      ...
      flushList();
      blocks.push({ type: "paragraph", html: formatInline(line) });
    }
    ...
    if (trimmed) {
      return [{ type: "paragraph", html: formatInline(trimmed) }];
    }
  ...
  <div class="notebooks-layout">
    ...
    <div class="notebooks-sidebar">
      <div class="sidebar-header">
        ...
        <button on:click={() => (showNewNotebook = true)} class="primary"> New Notebook </button>
      </div>
      ...
      {#if selectedNotebook}
        <div class="editor-header">
          ...
          <div class="editor-actions">
            <button on:click={() => (showNewCell = true)} class="secondary"> Add Cell </button>
              ...
              <div class="cell-header">
                <div class="cell-info">
                  ...
                  <span
                    ...
                    class:success={cell.status === "success"}
                ...
                <div class="cell-actions">
                  ...
                  <button
                    on:click={() => moveCell(cell.id, "down")}
                    disabled={index === selectedNotebook.cells.length - 1}
                    class="small"
                  >
                    ↓
                  </button>
                  <button on:click={() => deleteCell(cell.id)} class="small"> 🗑️ </button>
                </div>
              </div>
                  ...
                  <div class="markdown-cell">
                    ...
                    <div class="markdown-preview">
                      <div class="markdown-content" aria-live="polite">
                        {#each getMarkdownBlocks(cell.content) as block, index}
    ...
    <div class="dialog-overlay">
      <div class="dialog">
        ...
        <div class="dialog-actions">
          <button on:click={createNotebook} disabled={!newNotebookName.trim()}> Create </button>
          <button on:click={() => (showNewNotebook = false)} class="secondary"> Cancel </button>
        </div>
      </div>
    </div>
  {/if}
    ...
    <div class="dialog-overlay">
      <div class="dialog">
        ...
        <div class="dialog-actions">
          ...
          <button on:click={() => (showNewCell = false)} class="secondary"> Cancel </button>
        </div>
      </div>
    </div>
  {/if}
