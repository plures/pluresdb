<script lang="ts">
  ...
  async function executeQuery() {
    ...
    try {
      ...
      toast(
        `Query executed successfully (${queryResults.length} results, ${executionTime}ms)`,
        "success",
      );
    } catch (error) {
  ...
  <div class="query-builder-layout">
    ...
    <div class="queries-sidebar">
      <div class="sidebar-header">
        ...
        <button on:click={() => (showNewQuery = true)} class="primary"> New Query </button>
      </div>
      ...
      {#if selectedQuery}
        <div class="builder-header">
          ...
          <div class="builder-actions">
            ...
            <button on:click={saveQuery} class="secondary"> Save </button>
          ...
          {#if showRawMode}
            <div class="raw-editor">
              ...
              <div role="region" aria-label="Raw query editor">
                <JsonEditor {dark} value={rawQuery} onChange={(value) => (rawQuery = value)} />
              </div>
            </div>
          {:else}
                        ...
                        {#if condition.type === "field"}
                          <div class="field-condition">
                            <select
                              ...
                              on:change={() => updateCondition(index, condition)}
                            >
    ...
    <div class="dialog-overlay">
      <div class="dialog">
        ...
        <div class="dialog-actions">
          <button on:click={createQuery} disabled={!newQueryName.trim()}> Create </button>
          <button on:click={() => (showNewQuery = false)} class="secondary"> Cancel </button>
        </div>
      </div>
    </div>
  {/if}
